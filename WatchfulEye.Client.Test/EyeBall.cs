...
public class EyeBall : NetMQPeer {
    ...
    private async Task TakePicture(RequestPictureMessage message) {
        Logging.Debug("Sending test picture");
        if (SocketIP == null) {
        ...
        _isBusy = false;
        _currentThumbnailCount++;
        if (_currentThumbnailCount > 5)
    ...
    private async Task StreamVideo(RequestStreamMessage message) {
        Logging.Debug("Sending test video");
        if (SocketIP == null) {
            Logging.Error("Video has been requested but current Socket IP is null");
            return;
        }
    ...
    public static EyeBall? SocketEye(string eyeName) {
        ...
        // decode ack
        RegisterEyeAckMessage receiveMsg = (RegisterEyeAckMessage)MessageFactory.GetMessage(receiveData);
        if (receiveMsg == default) {
